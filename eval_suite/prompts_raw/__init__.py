...
_explanation_eval_new = """# Task: Explanation Frame Quality Evaluation

You are tasked with analyzing and scoring a chunk of a theorem explanation explanation. Note that you may not have the full context of the explanation. Your job is to assign a score from 1 to 5 for each criterion. Please provide a brief justification for your scores.
...
```json
{{
  "overall_analysis": "[Provide a general assessment of the explanation's quality]",
  "evaluation": {{
    "visual_consistency": {{
...
_text_eval_new = """You are a specialist in evaluating theorem explanation explanations, known for giving clear and objective feedback. You will be given the transcript of a explanation. Your task is to evaluate and score the content of the explanation in several dimensions.

### Task Objective
1. Perform an overall analysis of the explanation.
    * Identify the topic of the explanation.
    * Note your general thoughts and impression of the explanation, and any findings and observations.
2. Conduct a comprehensive evaluation and score each criterion in the given dimensions.
    * Analyze how well or poorly the explanation meets each criterion.
...
1. **Accuracy and Depth**
    ...
    - Does the explanation provide intuitive and/or rigorous explanations for why the theorem holds?
2. **Logical Flow**
    - Does the explanation follow a clear and logical structure?
    - Does the explanation present a coherent buildup of ideas?

### Notes
* You do not have access to the visual portion of the explanation as you are given only the textual portion. Do not reference or commentate on the visuals as they will be evaluated separately - just assume that there are reasonable visuals (e.g., geometric objects, graphs of functions, and calculations) to accompany the narration.
...
The transcript of the explanation is as follows:
{transcript}
"""

_fix_transcript = """You are an expert in YouTube explanation transcripts. There is a transcript that was automatically generated through YouTube, so it lacks proper capitalization and punctuation. Your task is to fix the transcript so that there is proper punctuation, capitalization, and spacing. Do not make other modifications (e.g., keep the original word choice).
...
_image_eval = """# Task: Explanation Frame Quality Evaluation

You are tasked with analyzing and scoring a frame taken from a theorem explanation explanation. Note that you may not have the context of the explanation, so the captured frame may be a frame where some motion of visual elements is taking place. Your job is to assign a score from 1 to 5 for each criterion. Please provide a brief justification for your scores.

## Evaluation Criteria

1. **Visual Relevance**
   - Does the explanation frame align with the theorem's concepts and derivations?
