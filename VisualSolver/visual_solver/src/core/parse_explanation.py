...
def get_images_from_explanation(explanation_path, fps=0.2):
...
def parse_srt_and_extract_frames(output_dir, topic_name):
    ...
    """
    topic_name = topic_name.replace(" ", "_").lower()
    explanation_path = os.path.join(output_dir, topic_name, f"{topic_name}_combined.mp4")
    srt_path = os.path.join(output_dir, topic_name, f"{topic_name}_combined.srt")
    subs = pysrt.open(srt_path)
    ...
    explanation = VideoFileClip(explanation_path)
...
def extract_trasnscript(explanation_path):
    ...
    audio_path = os.path.join(os.path.dirname(explanation_path), "audio.wav")
