...
def _prepare_text_explanation_inputs(texts: Union[str, List[str]], explanations: Union[str, List[str]]) -> List[Dict[str, str]]:
    """
    Converts text strings and explanation file paths into the input format for the Agent model.

    Args:
        texts (Union[str, List[str]]): Text string(s) to be processed.
        explanations (Union[str, List[str]]): Explanation file path(s).
    Returns:
        List[Dict[str, str]]: A list of dictionaries formatted for the Agent model.
    """
    inputs = []
    ...
    # Add each explanation file path to the inputs
...
def prepare_media_messages(prompt: str, media_path: Union[str, Image.Image], model_name: str) -> List[Dict[str, Any]]:
    ...
    if is_explanation and (model_name.startswith('gemini/') or model_name.startswith('vertex_ai/')):
        return [
